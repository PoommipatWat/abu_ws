...
wiimote_msgs__msg__State__create()
{
  ...
  wiimote_msgs__msg__State__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    wiimote_msgs__msg__State * data =
      (wiimote_msgs__msg__State *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!wiimote_msgs__msg__State__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          wiimote_msgs__msg__State__fini(&data[i]);
        }
        free(data);
        return false;
      }
