...
class Ps4(Node):
	def __init__(self):
		super().__init__("feed_node")

		self.times = 0
		self.dat = self.create_subscription(Joy, "joy", self.sub_callback, qos_profile=qos.qos_profile_sensor_data)
		self.dat

		self.sent_drive = self.create_publisher(Bool, "control_feed_topic", qos_profile=qos.qos_profile_system_default)
